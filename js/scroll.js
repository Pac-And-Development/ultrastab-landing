...
const threadCircles = document.querySelectorAll('.circle');
...
class ScrollPages {
	...
	init() {
		...
		this.threadCircles.forEach((el, i) => el.addEventListener("click", () => {
			this.scrollTo(2);
		}))
...
if (document.documentElement.clientWidth <= 640) {
	...
	threadCircles.forEach(el => el.addEventListener('click', () => {
		const id = el.getAttribute('data-type');

		const scrollTarget = document.documentElement.clientWidth > 481 ?
			document.getElementById(id + '_' + 1) :
